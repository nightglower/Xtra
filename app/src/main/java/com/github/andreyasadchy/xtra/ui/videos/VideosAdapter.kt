...
import com.github.andreyasadchy.xtra.ui.videos.game.GameVideosFragmentArgs
...
class VideosAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): PagingViewHolder {
        ...
        return PagingViewHolder(binding, fragment, clickListener, showDownloadDialog, saveBookmark)
    }
    ...
    inner class PagingViewHolder(
        ...
        private val clickListener: BaseVideosFragment.OnVideoSelectedListener,
        private val showDownloadDialog: (Video) -> Unit,
        private val saveBookmark: (Video) -> Unit): RecyclerView.ViewHolder(binding.root) {
        fun bind(item: Video?) {
            with(binding) {
                val context = fragment.requireContext()
                val channelClickListener: (View) -> Unit = { /*channelClickListener.viewChannel(item.user_id, item.user_login, item.user_name, item.channelLogo, streamId = item.id)*/ }
                val gameClickListener: (View) -> Unit = { fragment.findNavController().navigate(GameFragmentDirections.actionGlobalGameFragment(gameId = item?.gameId, gameName = item?.gameName)) }
                ...
                if (!item?.tags.isNullOrEmpty() && context.prefs().getBoolean(C.UI_TAGS, true)) {
                    ...
                    item?.tags?.forEach { tag ->
                        ...
                        text.text = tag.name
                        if (tag.id != null) {
                            text.setOnClickListener { fragment.findNavController().navigate(GameFragmentDirections.actionGlobalGameFragment(tags = listOf(tag.id).toTypedArray())) }
                        }

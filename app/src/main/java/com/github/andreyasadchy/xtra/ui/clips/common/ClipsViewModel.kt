...
class ClipsViewModel @Inject constructor(
    ...
    val sortText: LiveData<CharSequence>
        get() = _sortText
    private val args = ClipsFragmentArgs.fromSavedStateHandle(savedStateHandle)
    ...
    fun loadClips(context: Context) {
        ...
        filter.value = Filter(
            saveSort = sortValuesGame?.saveSort ?: sortValuesChannel?.saveSort ?: true,
    ...
    fun filter(context: Context, period: Period, languageIndex: Int, text: CharSequence, saveSort: Boolean, saveDefault: Boolean) {
        filter.value = Filter(saveSort = saveSort, period = period, languageIndex = languageIndex)
        ...
        viewModelScope.launch {
            if (!args.gameId.isNullOrBlank() || !args.gameName.isNullOrBlank()) {
                ...
                if (saveDefault != context.prefs().getBoolean(C.SORT_DEFAULT_GAME_CLIPS, false)) {
                    context.prefs().edit { putBoolean(C.SORT_DEFAULT_GAME_CLIPS, saveDefault) }
                }
            } else {
                if (!args.channelId.isNullOrBlank() || !args.channelLogin.isNullOrBlank()) {
                    ...
                    if (saveDefault != context.prefs().getBoolean(C.SORT_DEFAULT_CHANNEL_CLIPS, false)) {
                        context.prefs().edit { putBoolean(C.SORT_DEFAULT_CHANNEL_CLIPS, saveDefault) }
                    }
                }
            }
        }

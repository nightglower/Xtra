...
class ChatView : ConstraintLayout {
    ...
    private fun init(context: Context) {
        _binding = ViewChatBinding.inflate(LayoutInflater.from(context), this, false)
    }
    ...
    fun enableChatInteraction(enableMessaging: Boolean) {
        with(binding) {
            adapter.setOnClickListener { original, formatted, userId, channelId, host, fullMsg ->
                ...
                editText.clearFocus()
                findNavController().navigate(MessageClickedDialogDirections.actionGlobalMessageClickedDialog(enableMessaging = enableMessaging, originalMessage = original, formattedMessage = formatted, userId = userId, channelId = channelId, host = host, fullMsg = fullMsg))
            }
            if (enableMessaging) {
    ...
    class AutoCompleteAdapter(
        ...
        override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
            ...
            return when (getItemViewType(position)) {
                TYPE_EMOTE -> {
                    val binding = AutoCompleteEmotesListItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)
                    if (convertView == null) {
                        viewHolder = ViewHolder(binding.root).also { binding.root.tag = it }
                    } else {
